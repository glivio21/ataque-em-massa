(function() {
  ...
  if (1) {
    ...
    sortCoords = function() {
      ...
      for (var i = 0; i < len; i++) {
        ...
        for (var j = 0; j < lenAlvos; j++) {
          ...
          if (!j)
            prim = Math.sqrt(Math.pow(x[0] - y[0], 2) + Math.pow(x[1] - y[1], 2));
    ...
    firstRequest = function() {
      ...
      var spear = getParam('spear'), sword = getParam('sword'), axe = getParam('axe'), archer = getParam('archer'), spy = getParam('spy'), light = getParam('light'), heavy = getParam('heavy'), marcher = getParam('marcher'), ram = getParam('ram'), catapult = getParam('catapult'), snob = getParam('snob'), count = 0, i = 0;
    ...
    random = function(inferior, superior) {
      var numPossibilidades = superior - inferior, aleat = Math.random() * numPossibilidades;
      return Math.round(parseInt(inferior) + aleat);
    };
    main = function() {
      var coords = getParam('coords'), comando = getParam('comando'), i, tropas = "", href, spearIndex, snobIndex;
      ...
      if (!coords || !comando) {
        alert("Você precisa preencher as informações de aldeias e comando para iniciar o script.");
        return false;
      }
      usefullVillages();
      sortCoords();
